                ...
                >
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <span x-show="chat.unread > 0" class="bg-[#1a237e] text-white text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center" x-text="chat.unread"></span>
                            <button @click.stop="deleteChat(chat)" class="text-gray-400 hover:text-white hover:bg-red-500 transition-colors rounded-full p-1.5" title="Hapus percakapan">
                        ...
                        <div class="flex" :class="msg.from === 'customer' ? 'justify-end' : 'justify-start'">
                            <div class="relative max-w-[70%] min-w-[160px] group">
                                ...
                                <button @click.stop="toggleDropdown(msg)"
                                        :class="msg.from === 'customer' ? 'left-0 -translate-x-1/2' : 'right-0 translate-x-1/2'"
                                    ...
                                    <svg :class="openDropdownMsgId === msg.id ? 'rotate-0' : 'rotate-0'" class="w-3.5 h-3.5 text-gray-500 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
                                </button>
                                {{-- Bubble --}}
                                <div
                                ...
                                >
                                    ...
                                    <template x-if="msg.reply_to_id">
                                        <div :class="msg.from === 'customer' ? 'bg-blue-800/40 border-blue-600/30' : 'bg-gray-100 border-gray-200'"
                                             class="rounded-lg px-3 py-2 border-l-4 space-y-0.5 -mx-1">
                                            <p class="text-[11px] font-semibold" :class="msg.from === 'customer' ? 'text-blue-200' : 'text-gray-500'" x-text="'Membalas ' + (msg.reply_to_from === msg.from ? 'diri sendiri' : (msg.reply_to_from === 'admin' ? 'Admin' : 'Customer'))"></p>
                                            <p class="text-xs truncate" :class="msg.from === 'customer' ? 'text-blue-100' : 'text-gray-600'" x-text="msg.reply_to_text || '(file)'"></p>
                                        </div>
                                    </template>
                                    {{-- File attachment --}}
                                ...
                                <div x-show="openDropdownMsgId === msg.id"
                                     ...
                                     @click.stop
                                     :class="msg.from === 'customer' ? 'left-0' : 'right-0'"
                                ...
                                >
                                    <button @click="replyToMessage(msg); openDropdownMsgId = null" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2.5 transition-colors">
                                    ...
                                    <button @click="copyMessage(msg); openDropdownMsgId = null" x-show="msg.text" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2.5 transition-colors">
...
function chatApp() {
    return {
        ...
        async deleteMessage(msg) {
            ...
            try {
                const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
                const res = await fetch('/chat/message/' + msg.id, {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' }
                });
