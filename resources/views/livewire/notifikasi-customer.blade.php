    ...
    @else
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm divide-y divide-gray-100">
            @foreach($notifications as $notif)
                <div wire:click="markRead({{ $notif->id }})" class="p-5 hover:bg-gray-50 transition-colors cursor-pointer {{ !$notif->is_read ? 'bg-blue-50/30' : '' }}">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                <h3 class="font-semibold {{ !$notif->is_read ? 'text-gray-900' : 'text-gray-700' }}">{{ $notif->title }}</h3>
                            ...
                            @if($notif->data && isset($notif->data['order_number']))
                                <a href="{{ route('tracking', ['q' => $notif->data['order_number']]) }}" class="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline mt-2" wire:click.stop>
