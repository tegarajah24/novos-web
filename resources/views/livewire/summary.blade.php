<div>
    ...
    <div x-data="{ open: false }" class="bg-white rounded-xl border border-gray-200 shadow-sm mb-5">
        ...
        <div x-show="open" x-cloak x-transition class="px-6 pb-5 border-t border-gray-100">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-4">
                <div><label class="block text-xs text-gray-500 mb-1 font-medium">Assignee</label><select class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a237e]/30"><option>Semua</option></select></div>
                <div><label class="block text-xs text-gray-500 mb-1 font-medium">Periode</label><select class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a237e]/30"><option>Bulan Ini</option><option>Hari Ini</option><option>7 Hari Terakhir</option><option>30 Hari Terakhir</option><option>Custom</option></select></div>
                <div><label class="block text-xs text-gray-500 mb-1 font-medium">Prioritas</label><select class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a237e]/30"><option>Semua</option><option>Normal</option><option>Express</option><option>Super Express</option></select></div>
                <div><label class="block text-xs text-gray-500 mb-1 font-medium">Status</label><select class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a237e]/30"><option>Semua</option><option>Menunggu Verifikasi</option><option>Tahap Desain</option><option>Menunggu ACC</option><option>Produksi</option><option>Selesai</option></select></div>
                <div><label class="block text-xs text-gray-500 mb-1 font-medium">Tipe</label><select class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a237e]/30"><option>Semua</option><option>Custom</option><option>Produk Katalog</option></select></div>
            </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm mb-5 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                @php $loadColor = $e['load'] >= 75 ? 'bg-red-500' : ($e['load'] >= 50 ? 'bg-yellow-400' : 'bg-emerald-500'); @endphp
    ...
    new Chart(document.getElementById('chartTop5'), {
        ...
        data:{
            ...
            datasets:[{ label:'Terjual', data:@json($topProductData), backgroundColor:['#1a237e','#283593','#303f9f','#3949ab','#3f51b5'], borderRadius:6, barPercentage:0.7, categoryPercentage:0.85 }]
        },
        options:{
            ...chartDefaults, indexAxis:'y',
            plugins:{...chartDefaults.plugins, legend:{display:false}},
            animation:{ x:{from:0,duration:1200,easing:'easeOutQuart',delay:(ctx)=>ctx.dataIndex*150} },
            scales:{ x:{beginAtZero:true,grid:{color:'rgba(0,0,0,0.04)'},border:{display:false},ticks:{font:{size:11}}}, y:{grid:{display:false},border:{display:false},ticks:{font:{size:12,family:"'Poppins',sans-serif"}}} },
            hover:{mode:'nearest',intersect:true}
        },
        plugins:[
            { id:'rankingBadges', afterDatasetsDraw(chart){ const ctx=chart.ctx; const meta=chart.getDatasetMeta(0); const badges=['🥇','🥈','🥉']; meta.data.forEach((bar,index)=>{ if(index<3){ ctx.save(); ctx.font='16px serif'; ctx.textAlign='right'; ctx.textBaseline='middle'; ctx.fillText(badges[index],bar.base-12,bar.y); ctx.restore(); } }); } },
            { id:'valueLabels', afterDatasetsDraw(chart){ const ctx=chart.ctx; const meta=chart.getDatasetMeta(0); meta.data.forEach((bar,index)=>{ const value=chart.data.datasets[0].data[index]; ctx.save(); ctx.fillStyle='#1a237e'; ctx.font="bold 11px Poppins, sans-serif"; ctx.textAlign='left'; ctx.textBaseline='middle'; ctx.fillText(value+' terjual',bar.x+10,bar.y); ctx.restore(); }); } },
            { id:'hoverEnlarge', beforeDatasetsDraw(chart){ const active=chart.getActiveElements(); if(!active.length) return; const ctx=chart.ctx; active.forEach(el=>{ const {x,base,y,width}=el.element; const thickness=width*1.4; const newY=y-thickness/2; ctx.save(); ctx.fillStyle='rgba(26,35,126,0.12)'; ctx.fillRect(base,newY,x-base,thickness); ctx.restore(); }); } }
        ]
    });
